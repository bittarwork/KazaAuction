...
exports.newUser = (req, res) => {
  // validate requests by validtaing each input of the form
  ...
  if (req.body.password.length < 8)
    return res
      .status(400)
      .send({ message: "Password must be equal or more than 8 character!" });
  if (!req.body.gender)
  ...
  try {
    Schema.findOne({ email }).then((user) => {
      if (!user) {
        console.log("Email has not been used before.");
        ...
        return res
          ...
          .send({
            message:
              "Email have been registered, please login.",
          });
      }
    });
  } catch (err) {
    return res
      .status(500)
      .send({ message: err || "enternal Error with server" });
  }
};
  ...
  Schema.findById({ _id: req.params.id })
    ...
    .catch((err) => {
      if (err.kind === "ObjectId") {
        return res
          .status(404)
          .send({ message: "data not found with id " + req.params.id });
      }
      return res
        .status(500)
        .send({ message: "error retrieving data with id " + req.params.id });
    });
};
      ...
      Schema.findByIdAndUpdate({ _id: req.params.id }, newData, { new: true })
        ...
        .catch((err) => {
          if (err.kind === "Object_id") {
            return res
              .status(404)
              .send({ message: "data not found with _id " + req.params._id });
          }
          return res.status(500).send({
...
exports.findByIdAndRemove = (req, res) => {
  Schema.findByIdAndRemove({ _id: req.params.id })
    ...
    .catch((err) => {
      ...
      return res
        .status(500)
        .send({ message: "could not delete data with id " + req.params.id });
    });
};
...
exports.remove = (req, res) => {
  Schema.remove({})
    ...
    .catch((err) => {
      return res.status(500).send({ message: "Could not delete all data" });
    });
};
